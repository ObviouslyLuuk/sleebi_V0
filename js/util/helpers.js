...
if (document.body.dataset.mobile == 'false') {
    // Add css to adjust the scrollbar on desktop
...
function get_thumb_urls(video_id) {
    if (video_id == "demo480x852") {
        video_id = "NJoFdZFvfcc";
    };
    return {
        ...
        'maxres': get_thumb_url(video_id), // 1280x720
    };
};

function get_download_url(video_id) {
    if (video_id == "demo480x852") {
        return "https://sleebi.net/videos/demo480x852.mp4";
    }
...
function get_keywords(video_info) {
    let keywords = video_info['tags'];
    if (keywords == null)
        return [];
